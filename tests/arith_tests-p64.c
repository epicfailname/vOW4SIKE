...
#include "../src/P64/P64_internal.h"
...
{ // Tests for the field arithmetic
    ...
    printf("\n--------------------------------------------------------------------------------------------------------\n\n"); 
    printf("Testing field arithmetic over GF(p64): \n\n"); 

    // Field addition over the prime p64
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {
        fprandom64_test(a); fprandom64_test(b); fprandom64_test(c); fprandom64_test(d); fprandom64_test(e); fprandom64_test(f); 

        fpadd64(a, b, d); fpadd64(d, c, e);                 // e = (a+b)+c
        fpadd64(b, c, d); fpadd64(d, a, f);                 // f = a+(b+c)
        fpcorrection64(e);
        fpcorrection64(f);
    ...
    // Field subtraction over the prime p64
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {
        fprandom64_test(a); fprandom64_test(b); fprandom64_test(c); fprandom64_test(d); fprandom64_test(e); fprandom64_test(f); 

        fpsub64(a, b, d); fpsub64(d, c, e);                 // e = (a-b)-c
        fpadd64(b, c, d); fpsub64(a, d, f);                 // f = a-(b+c)
        fpcorrection64(e);
        fpcorrection64(f);
        if (compare_words(e, f, NWORDS_FIELD)!=0) { passed=0; break; }
    ...
    // Field multiplication over the prime p64
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {    
        fprandom64_test(a); fprandom64_test(b); fprandom64_test(c);  
        fprandom64_test(ma); fprandom64_test(mb); fprandom64_test(mc); fprandom64_test(md); fprandom64_test(me); fprandom64_test(mf); 
        ...
        fpmul64_mont(ma, mb, md); fpmul64_mont(md, mc, me);                          // e = (a*b)*c
        fpmul64_mont(mb, mc, md); fpmul64_mont(md, ma, mf);                          // f = a*(b*c)
        from_mont(me, e);
        from_mont(mf, f);
        if (compare_words(e, f, NWORDS_FIELD)!=0) { passed=0; break; }
      
        to_mont(a, ma); to_mont(b, mb); to_mont(c, mc); 
        fpadd64(mb, mc, md); fpmul64_mont(ma, md, me);                               // e = a*(b+c)
        fpmul64_mont(ma, mb, md); fpmul64_mont(ma, mc, mf); fpadd64(md, mf, mf);    // f = a*b+a*c
        from_mont(me, e);
        from_mont(mf, f);
        if (compare_words(e, f, NWORDS_FIELD)!=0) { passed=0; break; }
       
        to_mont(a, ma); to_mont(b, mb);
        fpmul64_mont(ma, mb, md);                                                      // d = a*b 
        fpmul64_mont(mb, ma, me);                                                      // e = b*a 
        from_mont(md, d);
        from_mont(me, e);
        if (compare_words(d, e, NWORDS_FIELD)!=0) { passed=0; break; }
        
        to_mont(a, ma);
        fpzero64(b); b[0] = 1; to_mont(b, mb);
        fpmul64_mont(ma, mb, md);                                                      // d = a*1  
        from_mont(ma, a);
        from_mont(md, d);                
        if (compare_words(a, d, NWORDS_FIELD)!=0) { passed=0; break; }
        
        fpzero64(b); to_mont(b, mb);
        fpmul64_mont(ma, mb, md);                                                      // d = a*0  
        from_mont(mb, b);
        from_mont(md, d);                
    ...
    // Field squaring over the prime p64
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {
        fprandom64_test(a);
        ...
        to_mont(a, ma);
        fpsqr64_mont(ma, mb);                                 // b = a^2
        fpmul64_mont(ma, ma, mc);                             // c = a*a 
        if (compare_words(mb, mc, NWORDS_FIELD)!=0) { passed=0; break; }

        fpzero64(a); to_mont(a, ma);
        fpsqr64_mont(ma, md);                                 // d = 0^2 
    ...
    // Field inversion over the prime p64
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {
        fprandom64_test(a); 
        to_mont(a, ma);
        fpzero64(d); d[0]=1; to_mont(d, md);
        fpcopy64(ma, mb);                            
        fpinv64_mont(ma);                                
        fpmul64_mont(ma, mb, mc);                             // c = a*a^-1 
...
{ // Tests for the quadratic extension field arithmetic
    ...
    printf("\n--------------------------------------------------------------------------------------------------------\n\n"); 
    printf("Testing quadratic extension arithmetic over GF(p64^2): \n\n"); 

    // Addition over GF(p64^2)
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {
        fp2random64_test((digit_t*)a); fp2random64_test((digit_t*)b); fp2random64_test((digit_t*)c); fp2random64_test((digit_t*)d); fp2random64_test((digit_t*)e); fp2random64_test((digit_t*)f); 

        fp2add64(a, b, d); fp2add64(d, c, e);                 // e = (a+b)+c
        fp2add64(b, c, d); fp2add64(d, a, f);                 // f = a+(b+c)
        if (compare_words((digit_t*)e, (digit_t*)f, 2*NWORDS_FIELD)!=0) { passed=0; break; }

        fp2add64(a, b, d);                                     // d = a+b 
        fp2add64(b, a, e);                                     // e = b+a
        if (compare_words((digit_t*)d, (digit_t*)e, 2*NWORDS_FIELD)!=0) { passed=0; break; }

        fp2zero64(b);
        fp2add64(a, b, d);                                     // d = a+0 
        if (compare_words((digit_t*)a, (digit_t*)d, 2*NWORDS_FIELD)!=0) { passed=0; break; }
        
        fp2zero64(b);
        fp2copy64(a, d);     
        fp2neg64(d);                      
        fp2add64(a, d, e);                                     // e = a+(-a)
    ...
    // Subtraction over GF(p64^2)
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {
        fp2random64_test((digit_t*)a); fp2random64_test((digit_t*)b); fp2random64_test((digit_t*)c); fp2random64_test((digit_t*)d); fp2random64_test((digit_t*)e); fp2random64_test((digit_t*)f); 

        fp2sub64(a, b, d); fp2sub64(d, c, e);                 // e = (a-b)-c
        fp2add64(b, c, d); fp2sub64(a, d, f);                 // f = a-(b+c)
        if (compare_words((digit_t*)e, (digit_t*)f, 2*NWORDS_FIELD)!=0) { passed=0; break; }

        fp2sub64(a, b, d);                                     // d = a-b 
        fp2sub64(b, a, e);                                         
        fp2neg64(e);                                           // e = -(b-a)
        if (compare_words((digit_t*)d, (digit_t*)e, 2*NWORDS_FIELD)!=0) { passed=0; break; }

        fp2zero64(b);
        fp2sub64(a, b, d);                                     // d = a-0 
        if (compare_words((digit_t*)a, (digit_t*)d, 2*NWORDS_FIELD)!=0) { passed=0; break; }
        
        fp2zero64(b);
        fp2copy64(a, d);                 
        fp2sub64(a, d, e);                                     // e = a+(-a)
        if (compare_words((digit_t*)e, (digit_t*)b, 2*NWORDS_FIELD)!=0) { passed=0; break; }
    }
    ...
    // Multiplication over GF(p64^2)
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {    
        fp2random64_test((digit_t*)a); fp2random64_test((digit_t*)b); fp2random64_test((digit_t*)c);  
        fp2random64_test((digit_t*)ma); fp2random64_test((digit_t*)mb); fp2random64_test((digit_t*)mc); fp2random64_test((digit_t*)md); fp2random64_test((digit_t*)me); fp2random64_test((digit_t*)mf); 
        ...
        fp2mul64_mont(ma, mb, md); fp2mul64_mont(md, mc, me);                          // e = (a*b)*c
        fp2mul64_mont(mb, mc, md); fp2mul64_mont(md, ma, mf);                          // f = a*(b*c)
        from_fp2mont(me, e);
        from_fp2mont(mf, f);
        if (compare_words((digit_t*)e, (digit_t*)f, 2*NWORDS_FIELD)!=0) { passed=0; break; }
      
        to_fp2mont(a, ma); to_fp2mont(b, mb); to_fp2mont(c, mc); 
        fp2add64(mb, mc, md); fp2mul64_mont(ma, md, me);                               // e = a*(b+c)
        fp2mul64_mont(ma, mb, md); fp2mul64_mont(ma, mc, mf); fp2add64(md, mf, mf);   // f = a*b+a*c
        from_fp2mont(me, e);
        from_fp2mont(mf, f);
        if (compare_words((digit_t*)e, (digit_t*)f, 2*NWORDS_FIELD)!=0) { passed=0; break; }
       
        to_fp2mont(a, ma); to_fp2mont(b, mb);
        fp2mul64_mont(ma, mb, md);                                                      // d = a*b 
        fp2mul64_mont(mb, ma, me);                                                      // e = b*a 
        from_fp2mont(md, d);
        from_fp2mont(me, e);
        if (compare_words((digit_t*)d, (digit_t*)e, 2*NWORDS_FIELD)!=0) { passed=0; break; }
        
        to_fp2mont(a, ma);
        fp2zero64(b); b[0][0] = 1; to_fp2mont(b, mb);
        fp2mul64_mont(ma, mb, md);                                                      // d = a*1  
        from_fp2mont(md, d);               
        if (compare_words((digit_t*)a, (digit_t*)d, 2*NWORDS_FIELD)!=0) { passed=0; break; }
        
        fp2zero64(b); to_fp2mont(b, mb);
        fp2mul64_mont(ma, mb, md);                                                      // d = a*0 
        from_fp2mont(md, d);               
    ...
    // Squaring over GF(p64^2)
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {
        fp2random64_test((digit_t*)a);
        
        to_fp2mont(a, ma);
        fp2sqr64_mont(ma, mb);                                 // b = a^2
        fp2mul64_mont(ma, ma, mc);                             // c = a*a 
        from_fp2mont(mb, b);               
        from_fp2mont(mc, c);               
        if (compare_words((digit_t*)b, (digit_t*)c, 2*NWORDS_FIELD)!=0) { passed=0; break; }

        fp2zero64(a); to_fp2mont(a, ma);
        fp2sqr64_mont(ma, md);                                 // d = 0^2 
        from_fp2mont(md, d);               
        if (compare_words((digit_t*)a, (digit_t*)d, 2*NWORDS_FIELD)!=0) { passed=0; break; }
    }
    ...
    for (n=0; n<TEST_LOOPS; n++)
    {
        ...
        to_fp2mont(a, ma);
        fp2zero64(d); d[0][0]=1; to_fp2mont(d, md);
        fp2copy64(ma, mb);                            
        fp2inv64_mont(ma);                                
        fp2mul64_mont(ma, mb, mc);                             // c = a*a^-1              
        from_fp2mont(mc, c);  
...
bool fp_run()
{
    ...
    printf("\n--------------------------------------------------------------------------------------------------------\n\n"); 
    printf("Benchmarking field arithmetic over GF(p64): \n\n"); 
        
    fprandom64_test(a); fprandom64_test(b); fprandom64_test(c);

    // GF(p) addition using p64
    cycles = 0;
...
bool fp2_run()
{
    ...
    printf("\n--------------------------------------------------------------------------------------------------------\n\n"); 
    printf("Benchmarking quadratic extension arithmetic over GF(p64^2): \n\n"); 
...
bool ecisog_run()
{
    ...
    for (n=0; n<BENCH_LOOPS; n++)
    {
        fp2random64_test((digit_t*)A24); fp2random64_test((digit_t*)C24);
    ...
    for (n=0; n<BENCH_LOOPS; n++)
    {
        fp2random64_test((digit_t*)A); fp2random64_test((digit_t*)coeff[0]); fp2random64_test((digit_t*)coeff[1]); fp2random64_test((digit_t*)coeff[2]);
    ...
    for (n=0; n<BENCH_LOOPS; n++)
    {
        fp2random64_test((digit_t*)A4); fp2random64_test((digit_t*)C);
    ...
    for (n=0; n<BENCH_LOOPS; n++)
    {
        fp2random64_test((digit_t*)A); fp2random64_test((digit_t*)C);
...
int main()
{
    ...
    OK = OK && fp_test();          // Test field operations using p64
    OK = OK && fp_run();           // Benchmark field operations using p64

    OK = OK && fp2_test();         // Test arithmetic functions over GF(p64^2)
    OK = OK && fp2_run();          // Benchmark arithmetic functions over GF(p64^2)

...
*
* Abstract: portable modular arithmetic for P64
*********************************************************************************************/
...
extern const uint64_t p64p1[NWORDS_FIELD]; 
...
{ // Efficient Montgomery reduction using comba and exploiting the special form of the prime p64.
  ...
  // ma is assumed to be in Montgomery representation.
    unsigned int i, j, carry, count = p64_ZERO_WORDS;
    ...
    for (i = 0; i < NWORDS_FIELD; i++) {
        for (j = 0; j < i; j++) {
            if (j < (i-p64_ZERO_WORDS+1)) { 
                MUL(mc[j], ((digit_t*)p64p1)[i-j], UV+1, UV[0]);
                ADDC(0, UV[0], v, carry, v); 
                ADDC(carry, UV[1], u, carry, u); 
                t += carry; 
            }
